...
#Author Yufeng Xi, this code is used for generating XRD pattern using cifs, apply different kinds of peak shape.
...
peak_shape_type = {
    ...
    'l': 'lorentzian',
    'v': 'voigt',
    'eg': 'empirical gaussian',
    'el': 'empirical lorentzian',
    'ev': 'empirical Pseudo-Voigt',
    'pp': 'pure peak'
}.get(pst)


work_dir = rf'C:\Users\xyfxy\Desktop\3_test\temp'
input_directory = os.path.join(work_dir,'cifs')

output_directory = os.path.join(work_dir,peak_shape_type)
skipped_dir = os.path.join(work_dir,f'skipped_{pst}')
pic_path = os.path.join(work_dir,'XRD_pattern')
...
for generated_file in os.listdir(output_directory):
    generated.add(os.path.splitext(generated_file)[0])
...
for filename in os.listdir(input_directory):
    ...
    if filename.endswith('.cif'):
        ...
        if  os.path.splitext(filename)[0] in generated:
        ...
        try:
            ...
            if any("www.crystallography.net" in line for line in lines):
                # Find indices of the required sentences
                multiplicity_index = next((i for i, line in enumerate(lines) if "_atom_site_symmetry_multiplicity" in line), None)
                fract_x_index = next((i for i, line in enumerate(lines) if "_atom_site_fract_x" in line), None)
            
                # Check if both sentences are found in the file
                if multiplicity_index is not None and fract_x_index is not None:
                    # Determine the order of the sentences and extract values accordingly
                    if multiplicity_index < fract_x_index:
                        # "_atom_site_symmetry_multiplicity" is before "_atom_site_fract_x"
                        ...
                        # "_atom_site_symmetry_multiplicity" is after "_atom_site_fract_x"
           ...
           # Calculating delta1 for each number in hkl_same and checking the condition
            ...
            # Define a peak shape function
            
            def pure_peak(theta_i, theta_k, intensity, std_dev = 0.1):
                #P_p = intensity * theta_i
                p_p = intensity * (1/np.sqrt(2*np.pi*std_dev**2)) * np.exp(-((theta_i - theta_k)**2) / (2*std_dev**2))
                return p_p
                
  
            def empirical_gaussian(theta_i, theta_k, intensity):
                H = np.sqrt(u * (np.tan(np.radians(theta_i)) ** 2) + v * np.tan(np.radians(theta_i)) + w)
                Hk = np.sqrt(u * (np.tan(np.radians(theta_k)) ** 2) + v * np.tan(np.radians(theta_k)) + w)
                x = (theta_i - theta_k) / Hk
                G_x = intensity * (np.sqrt(C_G) / (np.sqrt(np.pi) * H)) * np.exp(-C_G * x ** 2)
                return G_x
            
            def empirical_lorentzian(theta_i, theta_k, intensity):
                H_k = u / np.cos(np.radians(theta_k)) + v * np.tan(np.radians(theta_k))
                H_ = u / np.cos(np.radians(theta_i)) + v * np.tan(np.radians(theta_i))
                x = (theta_i - theta_k) / H_k
                L_x = intensity * (np.sqrt(C_L) / (np.pi * H_)) * (1 / (1 + C_L * x ** 2))
                return L_x
            
            def Pseudo_vogit(theta_i, theta_k, intensity):
                #eta = eta_0 + eta_1 * np.radians(theta_i) + eta_2 * np.radians(theta_i) ** 2  
                #eta_0 = 
                #eta_1 = np.random.rand(1)
               # eta_2 = np.random.rand(1)
               H = np.sqrt(u * (np.tan(np.radians(theta_i)) ** 2) + v * np.tan(np.radians(theta_i)) + w)
               Hk = np.sqrt(u * (np.tan(np.radians(theta_k)) ** 2) + v * np.tan(np.radians(theta_k)) + w)
               x = (theta_i - theta_k) / Hk
               eta = 0.5
               PV_x = eta * intensity * (np.sqrt(C_G) / (np.sqrt(np.pi) * H)) * np.exp(-C_G * x ** 2) + \
                   (1 - eta) * intensity * (np.sqrt(C_L) / (np.pi * H)) * (1 / (1 + C_L * x ** 2))
               return PV_x
                 
            
            
            def gaussian(theta_i, theta_k, intensity, std_dev = 0.1):
                #return amplitude * np.exp(-((x - mean) ** 2 / (2 * std_dev ** 2)))
                
                return intensity * (1/np.sqrt(2*np.pi*std_dev**2)) * np.exp(-((theta_i - theta_k)**2) / (2*std_dev**2))

        
            def lorentzian(theta_i, theta_k, intensity, FWHM = 0.1):
                
                return intensity * (1/np.pi) * (FWHM / 2) / ((theta_i - theta_k)**2 + (FWHM / 2)**2)

            def voigt(theta_i, theta_k, intensity, std_dev = 0.1, FWHM = 0.1):
                z = ((theta_i - theta_k) + 1j * FWHM) / (std_dev * np.sqrt(2))
                return intensity * np.real(wofz(z)) / (std_dev * np.sqrt(2 * np.pi))
            
            
            # Create Gaussian functions for each peak
            #gaussian_peaks = []
            
            x_new = np.linspace(5,89.99,8500)
            y_new = np.zeros_like(x_new)
            
            for theta_k, intensity in draw_I_Imax_percent:
                if intensity > 0:
                    if peak_shape_type == 'gaussian':
                        y_new += gaussian(x_new, theta_k, intensity)
                    elif peak_shape_type == 'lorentzian':
                        y_new += lorentzian(x_new, theta_k, intensity)
                    elif peak_shape_type == 'voigt':
                        y_new += voigt(x_new, theta_k, intensity)
                        
                    elif peak_shape_type == 'empirical gaussian':
                        y_new += empirical_gaussian(x_new, theta_k, intensity)
                    elif peak_shape_type == 'empirical lorentzian':
                        y_new += empirical_lorentzian(x_new, theta_k, intensity)
                    elif peak_shape_type == 'empirical Pseudo-Voigt':
                        y_new += Pseudo_vogit(x_new, theta_k, intensity)
                    elif peak_shape_type == 'pure peak':
                        
                        x_draw = draw_I_Imax_percent[(draw_I_Imax_percent[:, 0] >= 5) & (draw_I_Imax_percent[:, 0] <= 90), 0]
                        y_draw = draw_I_Imax_percent[(draw_I_Imax_percent[:, 0] >= 5) & (draw_I_Imax_percent[:, 0] <= 90), 1]
                        indices = np.searchsorted(x_new, x_draw)
                        y_new = np.zeros_like(x_new)
                        y_new[indices] = y_draw
                        
  
  
            # Normalize y values
            y_max = np.max(y_new)
            y_new_normalized = (y_new / y_max) * 1000
            y_new_normalized = np.round(y_new_normalized).astype(int)
            step_matrix = np.vstack((x_new, y_new_normalized)).T
            #print(step_matrix)
            ...
            df = pd.DataFrame(matrix_final_array)
            ...
            print(f"Saved {peak_shape_type} shape file: {output_filename}") 
            ...
            plt.title(os.path.splitext(filename)[0]+ f'_{peak_shape_type}')
            
            plt.savefig(os.path.join(pic_path, os.path.splitext(filename)[0] + '.png'))
            print(f"Saved {output_filename} picture") 
            plt.show()
        ...
        # identifying defected cif files
            ...
            shutil.move(cif_filepath,skipped_dir)
